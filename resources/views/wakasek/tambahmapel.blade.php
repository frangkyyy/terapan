            ...
            jamInput.addEventListener('input', function () {
                ...
                if (jamValue.length > 0) {
                    fetch(`/check-jam?jam=${encodeURIComponent(jamValue)}`)
                        ...
                        .then(data => {
                            if (data.exists) {
                                jamError.classList.remove('d-none'); // Tampilkan pesan error
                                submitButton.disabled = true; // Nonaktifkan tombol submit
                            } else {
                                jamError.classList.add('d-none'); // Sembunyikan pesan error
                                submitButton.disabled = false; // Aktifkan tombol submit
                            }
                        })
                        .catch(error => console.error('Error:', error));
                } else {
                    jamError.classList.add('d-none'); // Sembunyikan pesan error jika input kosong
                    submitButton.disabled = false; // Aktifkan tombol submit
                }
            });
